...
export default async function MovieCard({ movie, isFav }: MovieCardProps) {
  return (
    <article
    ...
    >
      <Link href={`/movies/${movie.id}`} className=" text-black block">
        ...
        <p
          ...
          data-testid="movie-year-director"
        >
          {movie.year} • {movie.director}
        </p>
        <p
          className="text-sm text-neutral-600"
          data-testid="movie-genre-rating"
        >
          {movie.genre} • ⭐ {movie.rating.toFixed(1)}
        </p>
        <p className="line-clamp-2 text-sm" data-testid="movie-desc">
          {movie.description}
        </p>
      </Link>
